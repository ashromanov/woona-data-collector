...
class PacketProcessingUpdateBatcherTest {
    @Test
    fun batchingController_preservesPacketArrivalClocks() {
        val delegate = TimestampCapturingPacketCaptureController()
        val controller = BatchingPacketCaptureController(
            delegate = delegate,
            batcher = PacketProcessingUpdateBatcher(
        ...
        controller.submit(byteArrayOf(1), 123L, 456L)

        assertEquals(123L, delegate.receivedAtMillis)
        assertEquals(456L, delegate.receivedAtMonotonicNs)
    }
...
private class TimestampCapturingPacketCaptureController : PacketCaptureController {
    var receivedAtMillis: Long? = null
    var receivedAtMonotonicNs: Long? = null

    override fun submit(packetFragment: ByteArray): PacketSubmitResult = error("Timestamp was dropped")
    ...
    override fun submit(
        ...
        this.receivedAtMonotonicNs = receivedAtMonotonicNs
